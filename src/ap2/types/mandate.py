...
class SecuritiesMandate(BaseModel):
    """Represents an instruction for securities movement (NSDL DIS equivalent)."""

    isin: str = Field(..., description="International Securities Identification Number.")
    quantity: int = Field(..., description="Number of units/shares to be moved.")
    instruction_type: str = Field(
        ..., description="Type of instruction: TRANSFER, PLEDGE, UNPLEDGE, etc."
    )
    target_dp_id: Optional[str] = Field(
        None, description="The ID of the target Depository Participant."
    )
    target_client_id: Optional[str] = Field(
        None, description="The ID of the target client account."
    )
    execution_date: str = Field(
        ..., description="The planned date for execution in ISO 8601 format."
    )
    reason_code: Optional[str] = Field(
        None, description="NSDL reason code for the transfer."
    )


class SecuritiesIntentMandate(IntentMandate):
    """An intent mandate specifically for securities operations."""

    securities_details: SecuritiesMandate = Field(
        ..., description="Specific details of the securities instruction."
    )
